...
import { getMyNewsFavoritesApi } from '@/api';
...
const loadMyFavorites = async () => {
  loading.value = true;
  try {
    const response = await getMyNewsFavoritesApi({
      page: page.value,
        ...
        <div class="flex flex-col h-full">
          ...
          <div class="space-y-2">
            ...
            <div class="text-xs text-gray-400">
              ...
              <div v-if="news.favoritedAt">收藏时间: {{ formatDate(news.favoritedAt) }}</div>
            </div>
          </div>
        </div>
      </ElCard>
